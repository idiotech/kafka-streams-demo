...
import tw.idv.idiotech.kafkastreams.avro.{ KeySerde, MockSchemaRegistry, ValueSerde }
...
trait MockKafkaStreams extends MockSchemaRegistry {
  ...
  def getInputTopic[K, V](topic: String)(
  ...
  def getOutputTopic[K, V](topic: String)(
    implicit testDriver: TopologyTestDriver,
    keySerde: KeySerde[K],
    valueSerde: ValueSerde[V]
  ): TestOutputTopic[K, V] =
